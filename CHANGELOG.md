...
- Dispatch preparation, start, completion, blocker, failure, and cancellation events are mirrored from durable attempt state into the ticket with idempotent markers. Final `DISPATCH_SUMMARY` fields are distilled into summary, files, tests, and flags while raw commands, prompt/output paths, logs, and full runtime metadata remain in attempt records.
...
- `livery link --force` now passes the force flag through to linked-repo harness entrypoint installation, matching the command's skip warning.
...
### Changed
- New workspace scaffolds use visibly Livery-owned harness names: Claude slash commands are grouped under `.claude/commands/livery/`, and Claude/Codex skills are named `livery-hello`, `livery-new-ticket`, and `livery-walkie-talkie`. This avoids occupying generic harness names such as `hello`, `ticket`, or `walkie`.
- `upgrade-workspace` backfills the new namespaced harness assets without deleting older generic assets from existing workspaces.
