...
COMMAND_HARNESS_ASSETS: tuple[CommandHarnessAsset, ...] = (
    ...
    CommandHarnessAsset(
        command="livery schedule new",
        skill_name="livery-schedule-new",
        description="Create a portable Livery schedule definition. Use when the user wants recurring, weekday, weekly, interval, or one-shot agent work.",
        summary="Create a git-tracked schedule definition without activating it on the host.",
        argument_hint="<schedule-id> --agent <id> (--daily HH:MM | --weekdays HH:MM | --weekly day --time HH:MM | --every duration | --at timestamp)",
        steps=(
            "Gather a stable schedule id, hired agent id, task, and exactly one trigger.",
            "Gather optional overlap, missed-run, and worktree policies when the defaults do not fit.",
            "Run `livery schedule new <schedule-id> --agent <id> --task \"...\" <trigger> [--overlap skip|allow] [--missed-run once|skip] [--worktree] --format json`.",
            "Report the definition path and remind the user that it is not installed or active yet.",
        ),
        notes=(
            "Daily and calendar times use the host's local timezone.",
            "Use an RFC 3339 timestamp with an explicit timezone for `--at`.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule list",
        skill_name="livery-schedule-list",
        description="List Livery schedule definitions and their host installation state. Use when the user asks what work is scheduled or installed.",
        summary="List tracked schedules with triggers, assignees, installation state, and last-run state.",
        steps=(
            "Run `livery schedule list --format json`.",
            "Summarize each schedule's id, trigger, assignee, installed state, and latest outcome.",
            "Distinguish a tracked definition from a native job installed on this host.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule install",
        skill_name="livery-schedule-install",
        description="Install or update one Livery schedule in the native user scheduler. Use when the user wants a tracked schedule activated on this Mac or Linux host.",
        summary="Preview, then install or update one schedule using launchd or systemd user services.",
        argument_hint="<schedule-id-or-fragment>",
        steps=(
            "Identify the schedule id or unique fragment.",
            "Run `livery schedule install <query> --dry-run --format json` and explain the backend, identity, command, and native files.",
            "Only when the user wants to activate it, run `livery schedule install <query> --format json`.",
            "Report the installed backend, identity, files, and enabled state.",
        ),
        notes=(
            "Installation changes per-user host scheduler state but does not require root.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule run",
        skill_name="livery-schedule-run",
        description="Run a Livery schedule through the managed dispatch lifecycle. Use when the user explicitly asks to run scheduled work now or when a native scheduler invokes it.",
        summary="Execute one schedule now and record it as a managed dispatch attempt.",
        argument_hint="<schedule-id-or-fragment> [--now]",
        steps=(
            "Identify the schedule id or unique fragment.",
            "For an explicit manual run, run `livery schedule run <query> --now --format json`.",
            "Report the outcome, dispatch attempt id, output path, and non-zero exit status if present.",
        ),
        notes=(
            "A manual run can launch an AI runtime and incur provider usage; do not run it merely to inspect a schedule.",
            "Native scheduler commands omit `--now` so occurrence deduplication remains active.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule status",
        skill_name="livery-schedule-status",
        description="Inspect one Livery schedule across its tracked definition, runtime state, and native scheduler state. Use for schedule health or last-run questions.",
        summary="Show portable, runtime, and native state for one schedule.",
        argument_hint="<schedule-id-or-fragment>",
        steps=(
            "Identify the schedule id or unique fragment.",
            "Run `livery schedule status <query> --format json`.",
            "Summarize whether it is installed and loaded, plus its latest attempt, outcome, and exit code.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule enable",
        skill_name="livery-schedule-enable",
        description="Enable an installed Livery schedule. Use only when the user explicitly asks to enable or resume a native scheduled job.",
        summary="Enable and load an installed native schedule.",
        argument_hint="<schedule-id-or-fragment>",
        steps=(
            "Confirm the schedule id or unique fragment.",
            "Run `livery schedule enable <query>`.",
            "Report that the native schedule is enabled.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule disable",
        skill_name="livery-schedule-disable",
        description="Disable an installed Livery schedule. Use only when the user explicitly asks to pause or disable a native scheduled job.",
        summary="Disable an installed native schedule while keeping its native files and tracked definition.",
        argument_hint="<schedule-id-or-fragment>",
        steps=(
            "Confirm the schedule id or unique fragment.",
            "Run `livery schedule disable <query>`.",
            "Report that the native schedule is disabled and its files remain installed.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule uninstall",
        skill_name="livery-schedule-uninstall",
        description="Uninstall a Livery schedule from the native user scheduler. Use only when the user explicitly asks to remove its host registration.",
        summary="Remove one native schedule registration while keeping its git-tracked definition.",
        argument_hint="<schedule-id-or-fragment>",
        steps=(
            "Confirm the schedule id or unique fragment and that the user wants its native registration removed.",
            "Run `livery schedule uninstall <query>`.",
            "Report that the native registration was removed and identify the preserved definition path.",
        ),
        notes=(
            "This does not delete `schedules/<id>.md`.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule sync",
        skill_name="livery-schedule-sync",
        description="Install or refresh every tracked Livery schedule on the current host. Use when the user wants host scheduler state synchronized with the workspace.",
        summary="Preview, then install or refresh all tracked schedules without removing orphaned native jobs.",
        steps=(
            "Run `livery schedule sync --dry-run --format json` and summarize the proposed installations.",
            "Only when the user wants to apply the changes, run `livery schedule sync --format json`.",
            "Report how many schedules were installed or refreshed and any failure.",
        ),
        notes=(
            "Sync never removes orphaned native jobs; uninstall those explicitly.",
        ),
    ),
    CommandHarnessAsset(
        command="livery schedule logs",
        skill_name="livery-schedule-logs",
        description="Read the latest output from a Livery schedule. Use when the user asks for scheduled task logs or recent run output.",
        summary="Tail the latest managed dispatch output for one schedule.",
        argument_hint="<schedule-id-or-fragment> [-n lines]",
        steps=(
            "Identify the schedule id or unique fragment and desired line count.",
            "Run `livery schedule logs <query> --lines <N>`.",
            "Relay the relevant output and explain when no run output exists yet.",
        ),
    ),
    CommandHarnessAsset(
        command="livery init",
